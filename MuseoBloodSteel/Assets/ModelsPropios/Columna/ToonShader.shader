Shader "Universal Render Pipeline/ToonWithOutline_Pro"
{
    Properties
    {
        ...
        _OutlineThickness ("Grosor Outline (unid. mundo)", Range(0.0005,0.02)) = 0.006
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float  ao     = lerp(1.0, SAMPLE_TEXTURE2D(_OcclusionMap, sampler_OcclusionMap, IN.uv).r, _OcclusionStrength);
                ...
                // Ambiente SH y occlusion
                float3 ambient = SampleSH(N) * _AmbientStrength * ao;

                // Specular toon (simple)
                float3 V = normalize(_WorldSpaceCameraPos - IN.positionWS);
                float3 H = normalize(L + V);
                float  NdotH = saturate(dot(N, H));
                float  specRaw = pow(NdotH, lerp(16.0, 64.0, _Metallic));   // más metálico = lóbulo más duro
                float  specQ   = toonQuantize(specRaw, _GlossSteps, 0.08, 0.4) * _SpecPower;

                float3 lit =
                    albedo.rgb * (ambient + diff * mainLight.color) +
                    _SpecColor.rgb * specQ;
